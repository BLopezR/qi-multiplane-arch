...
Recent proposals for QKD network management have explored the use of operational models that radically leverage the virtualization of the control and key management functionalities, and the disaggregation of the latter {{EVCK25}}. These approaches pave the way for a tighter integration of quantum security mechanism with functions already established in state of the art classical networks, including user authentication, service authorization, and mobility support, while maintaining compliance with established standards for QKD networks. Integrating these mechanisms enhances security measures and ensures that quantum networks can seamlessly interface with existing and future telecommunications infrastructure.

While SDN ensures higher degrees of flexibility and reconfigurability by allowing network functions to be easily modified and upgraded through software changes, virtualization enables the abstraction of hardware devices by creating virtual instances, which improves scalability, resource efficiency and allows the dynamic allocation of softwarized network functions in multi-domain environments. As quantum technology evolves, a virtualized layer for softwarized network functions significantly aids adaptation to these changes, ensuring pliability and responsiveness for seamless updates, and incorporating new mechanisms without extensive hardware modifications.

As for key exchange, current technology does not allow direct end-to-end quantum key exchange between distant nodes. Instead, key distribution must rely on trusted intermediary nodes to transmit keys hop-by-hop. A key management layer where the actions of all nodes are coordinated is needed to ensure secure and efficient key distribution. Virtualizing and decoupling key management from proprietary hardware through cloud-native virtualization enhances flexibility and scalability, and allows operators to gain the agility to implement hybrid cryptographic strategies that combine QKD with post-quantum algorithms - ensuring service continuity even during quantum channel outages. Additionally, it allows real-time performance monitoring, data-driven control and management, and tailored access and admission mechanisms {{QNSA24}}.
