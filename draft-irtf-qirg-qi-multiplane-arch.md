...
category: info
docname: draft-irtf-qirg-qi-multiplane-arch-latest
submissiontype: IRTF
number:
date:
consensus: true
...
venue:
  ...
  latest: "https://dr2lopez.github.io/qi-multiplane-arch/draft-irtf-qirg-qi-multiplane-arch.html"
author:
 - name: Diego Lopez
...
This document is intentionally a framework document: it does not prescribe a single protocol stack or a fixed layering. Instead, it provides a set of architectural anchors that allow new proposals to be positioned, compared, and discussed consistently. The document proposes a multi-plane reference architecture for the Quantum Internet, derived from available proposals and the operational experience with QKD infrastructure. Where this document draws from QKD practice, it does so explicitly; where it generalizes, it does so as a framework intended to host multiple quantum networking services beyond QKD.

The proposal attempts to define a framework with three essential properties to guarantee a seamless evolution of the technology, and the consolidation of applications and management practices:
...
# Base Technologies and Operational Experience: QKD and Evolved SDN Concepts
...
* A Service Stratum, dealing with the functionality related to the purpose of the quantum network, and aligned with SOP described as an example for QKD networks in the previous section. At this moment, the most general service, beyond QKD key management, is obviously entanglement distribution in a general quantum network. This stratum is intentionally defined in a technology and service-agnostic way. It does not assume a fixed layering or a single, primary service. In addition to QKD key management, used as a concrete experience base, candidate services include entanglement distribution, time synchronization, identity assurance, or sensing. The service stratum would consider the relevant service units (keys, shared states, identities, timelines...), deal with their appropriate distribution and routing, and deliver these service units as requested by the user application functions. The concept of service unit becomes essential here, as the cornerstone for fundamental network characteristics (addressing, routing, information structuring...) and for the interface to the applications using the network. The framework does not prescribe a single representation or packaging of service units; rather, it provides a place to discuss competing models (e.g., key-identifiers in a wide-area QKD network, or how to represent, request, and consume entanglement resources for, say, distributed computations or teleportation coding) and to relate them to routing, resource selection, and application-facing interfaces. 
...
[TBD] Reference clearly what other organizations (ETSI, ITU, etc.) are doing.
